...
import { fetchCandles, fetchSecondCandles } from '../../../src/api/quotation/candles';
import { DEFAULT_BASE_URL, CANDLE_PATH, CANDLE_SECONDS_PATH } from '../../../src/config/constants';
...
describe('Candles API', () => {
    ...
    describe('fetchSecondCandles', () => {
        ...
        it('should throw error if market is missing', async () => {
            await expect(fetchSecondCandles(http, { market: '' })).rejects.toThrow('Market is required for candles.');
        });
    });
});
